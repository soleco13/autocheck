...
// GET /api/reports/history — paginated list of all reports for this teacher
router.get('/history', requireAuth, async (req: AuthRequest, res: Response) => {
  ...
  const result = await db.query(`
    SELECT r.id, r.status, r.grade as report_grade, r.percentage, r.generated_at,
...
router.patch('/answers/:answerId/override', requireAuth, async (req: AuthRequest, res: Response) => {
  ...
  const result = await db.query(`
    ...
    WHERE id = $3
    RETURNING *
  `, [score, note, req.params.answerId]);
